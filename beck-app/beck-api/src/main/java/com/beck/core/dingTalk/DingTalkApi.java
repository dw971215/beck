...
public class DingTalkApi {
    ...
    public AjaxResult getUserInfo(@ApiParam(name = "code", value = "免登录码", required = true) String code,
        ...
        String token = DingTalkOauthUtil.getToken(appKey, appSecret);
        if(StringUtils.isNotBlank(token)){
            OapiV2UserGetuserinfoResponse.UserGetByCodeResponse userInfo = DingTalkOauthUtil.getUserInfo(code, token);
            if(userInfo == null){
